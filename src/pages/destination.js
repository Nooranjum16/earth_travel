...
export default function Destination() {
  ...
  return (
    <>
      <Head>
        <title>Earth Travel | Destination</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta property="og:title" content="Earth travel | Destination" />
            ...
            {data.planets.map((planet, i) => {
              return (
                <Image
                  ...
                  className={`${styles.planetsImg} ${planet.name}-img  ${
                    i === 0 ? "" : styles.imgHidden
                  }`}
                  key={planet.distance}
                />
              );
          ...
          <div className={styles.planetInfoContainer}>
            <div className={styles.planetsInfo}>
                  ...
                  {data.planets.map((planet) => {
                    return (
                      <li
                        key={planet.distance}
                        className={
                          linkActive === planet.name ? styles.linkNavActive : ""
                        }
                      >
                        <button onClick={handleNav} value={planet.name}>
                          {planet.name}
                        </button>
                      </li>
                    );
              ...
              {data.planets.map((planet, i) => (
                <article
                  key={planet.distance}
                  className={`${i === 0 ? "" : styles.sliderHidden} ${
                    planet.name + "-card"
                  }`}
                >
